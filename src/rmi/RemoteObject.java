...
public class RemoteObject extends UnicastRemoteObject implements
	...
	private Carga CargaToEntity(CargaDTO cargaDTO) {
		return new Carga(cargaDTO.getIdCarga(), cargaDTO.getPeso(),
				cargaDTO.getAncho(), cargaDTO.getAlto(),
				cargaDTO.getProfundidad(), cargaDTO.getVolumen(),
				cargaDTO.getFragilidad(), cargaDTO.getTratamiento(),
				cargaDTO.getApilable(), cargaDTO.isRefrigerable(),
				cargaDTO.getCondiciones(), cargaDTO.isDespachado(),
	...
	private Direccion DireccionToEntity(DireccionDTO direccionDTO) {
		return new Direccion(direccionDTO.getIdDireccion(),
				direccionDTO.getCalle(), direccionDTO.getNumero(),
				direccionDTO.getPiso(), direccionDTO.getDepartamento(),
				direccionDTO.getCP());
	}
	...
	private Empresa EmpresaToEntity(EmpresaDTO empresaDTO) {
		...
		return new Empresa(empresaDTO.getIdCliente(), empresaDTO.getNombre(),
				empresaDTO.getCUIT(), empresaDTO.getTipo(),
				empresaDTO.getDetallePoliticas(), productos,
				empresaDTO.getSaldoCuentaCorriente());
	}

	private Envio EnvioToEntity(EnvioDTO envioDTO) {
		Envio e = new Envio(envioDTO.getIdEnvio(), envioDTO.getFechaSalida(),
				envioDTO.getFechaLlegada(),
				envioDTO.isCumpleCondicionesCarga(), envioDTO.getEstado(),
				PedidoToEntity(envioDTO.getPedido()), envioDTO.getPrioridad());
	...
	private Factura FacturaToEntity(FacturaDTO facturaDTO) {
		return new Factura(facturaDTO.getIdFactura(),
				PedidoToEntity(facturaDTO.getPedido()), facturaDTO.getPrecio());
	}

	private Habilitado HabilitadoToEntity(HabilitadoDTO habilitadoDTO) {
		return new Habilitado(habilitadoDTO.getNombre(),
				habilitadoDTO.getDniHabilitado());
	}
	...
	private Particular ParticularToEntity(ParticularDTO particularDTO) {
		...
		return new Particular(particularDTO.getIdCliente(),
				particularDTO.getNombre(), particularDTO.getDNI(),
	...
	private Pedido PedidoToEntity(PedidoDTO pedidoDTO) {
		...
		Pedido p = new Pedido(pedidoDTO.getIdPedido(),
				DireccionToEntity(pedidoDTO.getDireccionCarga()),
				DireccionToEntity(pedidoDTO.getDireccionDestino()),
				pedidoDTO.getFechaCarga(), pedidoDTO.getHoraInicio(),
				pedidoDTO.getHoraFin(), pedidoDTO.getFechaMaxima(), cargas,
				pedidoDTO.getPrecio(), pedidoDTO.getSucursalDestino(),
				pedidoDTO.getSucursalOrigen(),
				pedidoDTO.isSolicitaTransporteDirecto(),
				pedidoDTO.isSolicitaAvionetaParticular(),
				ClienteToEntity(pedidoDTO.getCliente()));
		p.setSucursalOrigen(pedidoDTO.getSucursalOrigen());
		return p;
	}

	private PlanDeMantenimiento PlanDeMantenimientoToEntity(
			PlanDeMantenimientoDTO planDeMantenimientoDTO) {
		return new PlanDeMantenimiento(
				planDeMantenimientoDTO.getIdPlanDeMantenimiento(),
				planDeMantenimientoDTO.getDiasProxControl(),
				planDeMantenimientoDTO.getDiasDemora(),
				planDeMantenimientoDTO.getKmProxControl());
	}

	private PrecioVehiculo PrecioVehiculoToEntity(
			PrecioVehiculoDTO precioVehiculoDTO) {
		return new PrecioVehiculo(precioVehiculoDTO.getIdPrecioVehiculo(),
				precioVehiculoDTO.getTipoVehiculo(),
				precioVehiculoDTO.getPrecio());
	}

	private Producto ProductoToEntity(ProductoDTO productoDTO) {
		return new Producto(productoDTO.getIdProducto(),
				productoDTO.getNombre(), productoDTO.getTipo());
	}
	...
	private Remito RemitoToEntity(RemitoDTO remitoDTO) {
		return new Remito(remitoDTO.getIdRemito(),
				PedidoToEntity(remitoDTO.getPedido()));
	}
	...
	private Sucursal SucursalToEntity(SucursalDTO sucursalDTO) {
		return new Sucursal(sucursalDTO.getIdSucursal(),
	...
	private Transporte TransporteToEntity(TransporteDTO transporteDTO) {
		return new Transporte(transporteDTO.getIdProveedor(),
				transporteDTO.getCompania(), transporteDTO.getTipoMercaderia(),
				transporteDTO.getTipoTransporte());
	}

	private Trayecto TrayectoToEntity(TrayectoDTO trayectoDTO) {
		return new Trayecto(trayectoDTO.getIdTrayecto(),
				SucursalToEntity(trayectoDTO.getSucursalOrigen()),
				SucursalToEntity(trayectoDTO.getSucursalDestino()),
				trayectoDTO.getTiempo(), trayectoDTO.getKm(),
				trayectoDTO.getPrecio());
	}

	private Vehiculo VehiculoToEntity(VehiculoDTO vehiculoDTO) {

		return new Vehiculo(vehiculoDTO.getIdVehiculo(), vehiculoDTO.getTipo(),
				vehiculoDTO.getVolumen(), vehiculoDTO.getPeso(),
				vehiculoDTO.getAncho(), vehiculoDTO.getAlto(),
				vehiculoDTO.getProfundidad(), vehiculoDTO.getTara(),
				vehiculoDTO.getKilometraje(), vehiculoDTO.getEstado(),
				vehiculoDTO.isEnGarantia(), vehiculoDTO.isTrabajoEspecifico(),
				vehiculoDTO.getFechaUltimoControl(),
				PlanDeMantenimientoToEntity(vehiculoDTO
						.getPlanDeMantenimiento()));
	...
	public void altaPlanMantenimiento(
			PlanDeMantenimientoDTO planDeMantenimientoDTO) {
	...
	public boolean ControlarVehiculo(VehiculoDTO vehiculoDTO) {
		...
		c.add(Calendar.DATE, vehiculoDTO.getPlanDeMantenimiento()
				.getDiasProxControl());
		Date fecha = c.getTime();

		boolean Boolean = false;
		if (vehiculoDTO.getEstado().equals("En Deposito")) {
			System.out.println(vehiculoDTO.getKilometraje()
					+ vehiculoDTO.getPlanDeMantenimiento().getKmProxControl());
			if (vehiculoDTO.getKilometraje() >= vehiculoDTO
					.getPlanDeMantenimiento().getKmProxControl()) {
				Boolean = true;
				...
				vehiculoDTO.getPlanDeMantenimiento().setKmProxControl(
						vehiculoDTO.getKilometraje() + 200);
	...
	private List<TrayectoDTO> obtenerTodosLosTrayectos(SucursalDTO origen,
			SucursalDTO destino) {
		List<TrayectoDTO> trayectos = new ArrayList<TrayectoDTO>();

		for (RutaDTO r : mapadeRuta.getRutas()) {
			for (TrayectoDTO t : r.getTrayectos()) {
				if (t.getSucursalOrigen() == origen
						&& t.getSucursalDestino() == destino) {
					trayectos.add(t);
				}
			}
		}
	...
	private List<RutaDTO> obtenerTodasLasRutasDirectas(
			SucursalDTO sucursalOrigen) {
		MapaDeRutaDTO mp = mapadeRuta;
		List<RutaDTO> rutas = new ArrayList<RutaDTO>();
		for (RutaDTO r : mp.getRutas()) {
			if (r.getOrigen().getIdSucursal() == sucursalOrigen.getIdSucursal()
					&& r.getTrayectos().size() == 1) {
				rutas.add(r);
			}
		}
	...
	public RutaDTO obtenerMejorRuta(SucursalDTO origen, SucursalDTO destino) {
		...
		for (RutaDTO r : mp.getRutas()) {
			...
			if (r.getOrigen().getIdSucursal() == origen.getIdSucursal()
					&& r.getDestino().getIdSucursal() == destino
							.getIdSucursal()) {
		...
		for (PedidoDTO p : pedidos) {
			...
			if (!envio.getEstado().equals("despachado")) {
				if (envio.getEstado().equals("listo")) {
					...
					if (!nul) {
						ViajeDTO viajeAux = hbtDAO.obtenerViajeDeEnvio(envio
								.getIdEnvio());
					...
					if (!nul)
						sucursalOrigen = obtenerSucursal(envio2
								.getSucursalOrigen());

					envio2.setSucursalOrigen(String.valueOf(sucursalOrigen
							.getIdSucursal()));
					envio = envio2;
				}
				...
				if (vehiculos != null) {
					...
					if (carga > volu && carga < v.getVolumen()) {
						...
						viaje.setSucursalDestino(ruta.getTrayectos().get(0)
								.getSucursalDestino());
						viaje.setVehiculo(v);
						...
						Date auxiliar = Calendar.getInstance().getTime();
						long minutosAux = m * milisegundos;
						Date auxiliar2 = new Date(auxiliar.getTime()
								+ minutosAux);
					...
					for (VehiculoDTO v : vehiculos) {
						if (v.getEstado().equals("En Deposito")) {
							...
							if (carga > volu && carga < v.getVolumen()) {
								...
								long m = (long) tiempo;
								long milisegundos = 60000;
								Date auxiliar = Calendar.getInstance()
										.getTime();
								long minutosAux = m * milisegundos;
								Date auxiliar2 = new Date(auxiliar.getTime()
										+ minutosAux);
								...
								for (EnvioDTO e : en) {
									m = (long) ruta.getTrayectos().get(0)
											.getTiempo();
									milisegundos = 60000;
									auxiliar = Calendar.getInstance().getTime();
									minutosAux = m * milisegundos;
									auxiliar2 = new Date(auxiliar.getTime()
											+ minutosAux);
	...
	private List<PedidoDTO> ordenarPedidosPorPrioridad(List<PedidoDTO> pedidos) {
		...
		for (int i = 0; i < pedidos.size(); i++) {
			 ...
			 * if(!pedidos
			 * .get(j).getFechaMaxima().before(viajeDTO.getFechaLlegada())) {
			 * respuesta=enviarUrgente(pedidos.get(j)); if(respuesta) {
			 * pedidos.remove(i); } } else if(!respuesta) {
			 */
			for (int j = 0; j < pedidos.size(); j++) {
				if (pedidos.get(j).getFechaMaxima()
						.before(pedidos.get(i).getFechaMaxima())) {
					/*
	...
	public void recibir(ViajeDTO v) {
		...
		for (EnvioDTO e : v.getEnvios()) {
			if (Integer.parseInt(e.getPedido().getSucursalDestino()) == v
					.getSucursalDestino().getIdSucursal()) {
				for (CargaDTO c : e.getPedido().getCargas()) {
		...
		VehiculoDTO veh = hbtDAO.obtenerVehiculo(v.getVehiculo()
				.getIdVehiculo());
